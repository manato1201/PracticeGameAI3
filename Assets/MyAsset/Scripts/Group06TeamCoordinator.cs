...
namespace Group06
{
    public class Group06TeamCoordinator : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            tp = new Vector3[n];
            for (int i = 0; i < n; i++) tp[i] = GameManager.instance.TargetPosition(i);
        ...
        private void BuildDistanceTable()
        {
            ...
            Vector3 a0 = agents[0].GetPosition(); a0.y = 0;
            Vector3 a1 = agents[1].GetPosition(); a1.y = 0;

            // start -> target
            for (int t = 0; t < n; t++)
            {
                Vector3 g = tp[t]; g.y = 0;
            ...
            for (int i = 0; i < n; i++)
            {
                Vector3 si = tp[i]; si.y = 0;
                for (int j = 0; j < n; j++)
                {
                    if (i == j) { dist[2 + i, 2 + j] = 0f; continue; }
                    Vector3 gj = tp[j]; gj.y = 0;
        ...
        private (float cost, int[] order) SolveBestOrder(int startNode, int maskTargets)
        {
            ...
            for (int i = 0; i < dp.Length; i++) { dp[i] = float.PositiveInfinity; parent[i] = -1; }
            ...
            for (int last = 0; last < M; last++)
            {
                ...
                if (v < best) { best = v; bestLast = last; }
            }
        ...
        public int GetNextTarget(int agentId)
        {
            ...
            while (planPos[agentId] < plan[agentId].Length)
            {
                ...
                if (GameManager.instance.IsTargetActive(t)) return t;
                planPos[agentId]++;
            }
            return -1;
        }
        ...
        public bool IsTargetStillActive(int index) => GameManager.instance.IsTargetActive(index);
