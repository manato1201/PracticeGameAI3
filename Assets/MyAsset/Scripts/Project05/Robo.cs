...
namespace Project05
{
    ...
    public class Robo : Pawn_Project05
    {
        public int set = 0;
        ...
        void Start()
        {
            numTargets = GameManager_Project05.instance.NumTargets();
            targetPositions = new Vector3[numTargets];
            hitStatus = new bool[numTargets];
            for (int i = 0; i < numTargets; i++)
            {
                targetPositions[i] = GameManager_Project05.instance.TargetPosition(i);
                hitStatus[i] = false;
            }
            ...
            switch (step)
            {
                case 0:
                    ...
                    if (set == 0)
                    {
                        for (int i = 0; i < numTargets; i++)
                        {
                            if (GameManager_Project05.instance.IsTargetActive(i))
                            {
                                nextTarget = i;
                                break;
                            }
                    ...
                    UnityEngine.AI.NavMesh.CalculatePath(GetPosition(), agent.destination,
...

                {
                    ...
                    GameManager_Project05.instance.DispStr("Next:" + targetPos);
                    Debug.DrawLine(nextPos, GetPosition(), Color.green);
                    GameManager_Project05.instance.DispStr("toNext:" + delta.magnitude);
                    SetDirection(delta);
                    SetMoveSpeed(5f);
                    ...
                    {
                        GameManager_Project05.instance.DispStr("toAlly:" + r);
